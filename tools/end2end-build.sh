...
# 1 - build directory
pull() {
	svn co https://metanas.svn.sourceforge.net/svnroot/metanas/$BRANCH $1
}
...
post_remote_files() {
	...
	case "$BRANCH" in
	trunk)
		branch_reldir=MetaNAS-8-nightly
		;;
	*)
		# e.g. MetaNAS-8.2.0/x86
		branch_reldir=MetaNAS-${RELEASE%%-*}/$arch
		;;
	esac
	_do scp -o BatchMode=yes $* \
	    ${SFUSER:-yaberauneya},metanas@frs.sourceforge.net:/home/frs/project/f/fr/metanas/$branch_reldir
}
...
do
	...
	do
		...
		_do sudo sh -c "export METANAS_ARCH=$_ARCH; env $BUILD_PASS1_ENV $BUILD && env $BUILD_PASS2_ENV $BUILD" > $_LOG 2>&1
		_EC=$?
